 ...
 *
 * Porté depuis src/lib/curriculum.ts du monolithe Next.js (types TypeScript
 * retirés, logique et données identiques). Ce module est l'unique source de
 * vérité du programme utilisée par buildSystemPrompt (promptBuilder.js) afin
 * que l'assistant suive strictement le programme ivoirien plutôt qu'un
 * contenu générique.
 */
...
/**
 * Normalise un libellé de niveau scolaire vers le format abrégé attendu par
 * CURRICULUM ("TLE", "2NDE", "6EME", "CM2", ...).
 *
 * Nécessaire car gradeLevel peut arriver sous des formats très différents
 * selon la source : l'attribut Keycloak "grade_level" est un texte libre
 * défini par l'admin (ex. "Terminale D", avec la série incluse), alors que
 * CURRICULUM est indexé sur les codes abrégés du monolithe d'origine. Un
 * futur formulaire frontend pourrait aussi envoyer "Terminale" tout court,
 * "Tle", etc. Cette fonction absorbe ces variations pour que
 * buildCurriculumContext() retrouve toujours les bons chapitres plutôt que
 * d'échouer silencieusement.
 *
 * Idempotente : si gradeLevel est déjà au format abrégé ("TLE", "6EME"...),
 * elle le retourne tel quel.
 */
export function normalizeGradeLevel(gradeLevel) {
  if (!gradeLevel) return gradeLevel;

  const KNOWN_CODES = ["CP1", "CP2", "CE1", "CE2", "CM1", "CM2", "6EME", "5EME", "4EME", "3EME", "2NDE", "1ERE", "TLE"];
  ...
  // Format non reconnu : on retourne tel quel (getChapters renverra [] et
  // buildCurriculumContext omettra simplement le bloc "Programme officiel").
  return gradeLevel;
}

export function getCycle(gradeLevel) {
...
export function getChapters(subject, gradeLevel) {
  return CURRICULUM[subject]?.[normalizeGradeLevel(gradeLevel)] ?? [];
}
 ...
 */
export function buildCurriculumContext(gradeLevel, subject, serie) {
  const parts = [APC_FRAMEWORK, "", IVORIAN_CONTEXT];
